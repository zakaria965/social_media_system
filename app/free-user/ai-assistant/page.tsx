...
export default function FreeAIAssistantPage() {
  ...
  return (
    <div className="max-w-[700px] mx-auto w-full flex flex-col min-h-[calc(100vh-140px)] space-y-6 px-4 py-4">
      {/* Top Header Section */}
      <div className="border-b border-slate-200/40 dark:border-white/5 pb-6 select-none">
      ...
      <div className="flex-1 flex flex-col min-h-[350px] bg-transparent rounded-2xl overflow-hidden relative">
        {messages.length === 0 ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center space-y-5 py-12 select-none my-auto max-w-[500px] mx-auto">
            <div className="p-4 bg-white dark:bg-[#1F2937] rounded-3xl shadow-[0_4px_20px_rgba(15,23,42,0.03)] border border-slate-200/40 dark:border-white/5 text-[#22C55E]">
          ...
          <div className="flex-1 overflow-y-auto space-y-6 pr-1 max-h-[550px] pb-6">
                ...
                >
                  <div
                    className={`max-w-[85%] px-4.5 py-3 text-sm font-medium leading-relaxed ${
                      isUser
                        ? "bg-[#F0FDF4] dark:bg-emerald-950/20 text-slate-800 dark:text-slate-100 rounded-2xl rounded-tr-none ml-auto shadow-xs border border-emerald-500/10 font-semibold"
                        : isError
                        ? "bg-red-50/60 dark:bg-rose-950/20 text-red-650 dark:text-red-400 rounded-2xl rounded-tl-none shadow-xs border border-red-500/10 font-semibold"
                        : "bg-[#FCFAF6] dark:bg-[#1F2937]/60 text-slate-800 dark:text-slate-100 rounded-2xl rounded-tl-none shadow-xs border border-slate-200/40 dark:border-white/5"
                    }`}
                  >
            ...
            {loading && (
              <div className="flex flex-col items-start w-full space-y-1 animate-pulse">
                <div className="max-w-[85%] px-4.5 py-3 bg-[#FCFAF6] dark:bg-[#1F2937]/60 text-slate-800 dark:text-slate-100 rounded-2xl rounded-tl-none shadow-xs border border-slate-200/40 dark:border-white/5">
                  <div className="flex items-center gap-2">
                    <RefreshCw className="size-4 text-[#22C55E] animate-spin" />
                    <span className="text-sm font-semibold text-slate-800 dark:text-white">Thinking...</span>
                  </div>
                  <p className="text-[10px] text-slate-450 dark:text-slate-400 mt-1 font-bold">Powered by Gemini</p>
                </div>
              </div>
            )}
            <div ref={chatEndRef} />
      ...
      {remaining > 0 ? (
        <div className="bg-white dark:bg-[#111827] rounded-3xl p-5 shadow-[0_8px_30px_rgba(15,23,42,0.04)] border border-slate-200/40 dark:border-white/5 space-y-4">
          {remaining === 1 && (
            <div className="bg-amber-50/60 dark:bg-amber-950/20 border border-amber-500/10 rounded-2xl p-4 flex items-center gap-2.5 text-amber-700 dark:text-amber-300 text-xs font-bold select-none">
          ...
          <div className="space-y-1">
            <Textarea
              ...
              className="text-sm border border-slate-200/40 dark:border-white/5 bg-[#FCFAF6] dark:bg-[#1F2937]/40 rounded-2xl focus-visible:ring-[#22C55E]/40 focus-visible:border-[#22C55E] resize-none p-4 placeholder:text-slate-400 dark:placeholder:text-slate-500 text-slate-800 dark:text-slate-100 font-semibold w-full outline-hidden h-[140px] min-h-[140px] max-h-[140px]"
            />
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
              {suggestionChips.map((chip) => (
                <button
                  ...
                  disabled={remaining <= 0}
                  className="text-[11px] font-bold bg-[#FCFAF6] dark:bg-[#1F2937]/40 text-slate-700 dark:text-slate-200 px-3 py-2.5 rounded-xl border border-slate-200/40 dark:border-white/5 hover:bg-[#F0FDF4] dark:hover:bg-[rgba(48,252,71,0.08)] hover:text-[#22C55E] dark:hover:text-[#22C55E] transition-all text-center truncate disabled:opacity-40 cursor-pointer"
                  title={chip.label}
                >
                  {chip.label}
                </button>
        ...
        <div className="bg-white dark:bg-[#111827] rounded-3xl p-8 shadow-[0_8px_30px_rgba(15,23,42,0.04)] border border-slate-200/40 dark:border-white/5 text-center space-y-6 select-none w-full">
          <div className="inline-flex size-12 items-center justify-center rounded-2xl bg-red-50/60 dark:bg-red-950/20 text-red-500 dark:text-red-400">
            <Sparkles className="size-6 fill-current animate-pulse" />
          </div>
          <div className="space-y-2">
            <h3 className="text-lg font-black text-slate-800 dark:text-white">AI Credit Limit Reached</h3>
          ...
          <div className="flex justify-center gap-3">
            ...
            <Button
              ...
              className="bg-slate-100 dark:bg-[#1F2937]/60 hover:bg-slate-200/60 dark:hover:bg-slate-700/60 text-slate-700 dark:text-slate-300 font-black text-xs uppercase tracking-wider px-8 py-3.5 rounded-full border border-slate-200/40 dark:border-white/5 shadow-sm transition-all cursor-pointer font-bold"
            >
              Contact Support
            </Button>
          </div>
